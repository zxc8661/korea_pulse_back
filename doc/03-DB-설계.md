...
- 스키마 변경은 Flyway로만 한다. `V1__init.sql`에 아래 15개 테이블을 담고 JPA `ddl-auto`는 `validate`로 고정한다.
- 기사 본문은 어떤 테이블에도 저장하지 않는다. 공급자가 돌려준 제목, 스니펫, 링크, 발행 시각, 출처까지만 보관한다(01 문서 9절).
...
15개 테이블. 실선은 FK, 점선은 문자열 키로만 이어지는 논리 관계다.
...
| 1 후보 | collection_run (candidate_count, stats), topic(last_collected_at) |
...
| 9 점수 | (없음, 메모리) |
| 10 검색 관심도 | search_interest_snapshot |
...
| is_primary | BOOLEAN | NOT NULL DEFAULT false | 대표 키워드 여부. DataLab 조회 대상 |
| last_seen_at | TIMESTAMPTZ | NOT NULL | |
...
DataLab 조회 결과. 조회 조건까지 함께 보관한다(정의서 §5.4).

| 컬럼 | 타입 | 제약 | 설명 |
| --- | --- | --- | --- |
| keyword | VARCHAR(50) | PK | 대표 키워드(topic_keyword.is_primary) |
| stat_date | DATE | PK | 지수의 기준 일자 (KST) |
| ratio | NUMERIC(5,2) | NULL, CHECK(0~100) | 상대 지수. 값이 없으면 NULL |
| keyword_group | JSONB | NOT NULL | 조회에 사용한 키워드 그룹 |
| period_from / period_to | DATE | NOT NULL | 조회 기간 |
| unavailable_reason | VARCHAR(12) | NULL, CHECK(NO_DATA/QUOTA/ERROR) | ratio가 NULL일 때 사유 |
| fetched_at | TIMESTAMPTZ | NOT NULL | 마지막 조회 시각 |

timeUnit은 date/week/month뿐이므로 최소 단위는 date다. 시간 단위 컬럼을 추가하거나 일 값을 24등분해 백필하는 일은 금지한다. 서로 다른 `keyword_group`·`period` 조합의 ratio를 직접 비교하지 않는다.
...
| search_interest_snapshot | ≤ 200 | ~120 B | 무시 | 연 ~9 MB |
